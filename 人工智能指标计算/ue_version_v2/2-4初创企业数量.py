...
for field in field_list:
    ...
    SELECT
        province,
        substr(found_year,1,4) as found_year
    FROM
        kjb_de_company
    WHERE
       label LIKE '%{}%'  
    and not ISNULL(province)
    and not ISNULL(found_year) 
    and found_year != ''
    and province != ''
    '''.format(field)
    ...
    if len(df_SQL) > 0:
        ...
        df_SQL = df_SQL.loc[df_SQL["found_year"] != "0000"]
        ...
        df_group = df_SQL_all.groupby(by=["year","new_province"]).count().reset_index()

        df_pivot = pd.pivot_table(df_group, index="year", columns="new_province", values="province",aggfunc=np.sum)
...
cursor.execute('delete from industry_develop_index where index_num ="2_4"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='2_4'
'''
cursor.execute(sql)
