...
for field in field_list:
    ...
    if len(df_SQL) > 0:
        ...
        for i in range(0, df.shape[1]):
            # i=0
            # print(i)
            if i == 0 and int(a[i]) >= 2016 and int(a[i]) < now_year:
                df1 = df.iloc[:, i]
...
cursor.execute('delete from industry_develop_index where index_num ="2_2"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='2_2'
'''
cursor.execute(sql)
