...
for field in field_list:
    ...
    FROM
        (
        SELECT
            SUBSTR( public_date, 1, 4 ) AS YEAR,
            applicant_address_other AS province
        FROM
            ai_pct
        WHERE
            field LIKE "%{}%"
            AND applicant_country_code LIKE "中国"
        ) a
    WHERE
        province != ""
    GROUP BY
    ...
    if len(df_SQL) > 0:
        ...
        for province in province_list:
            ...
            for i in range(len(df_SQL)):
                if province in df_SQL["province"][i]:
                    df_SQL["new_province"][i] = province
        ...
        df_pivot = pd.pivot_table(df_SQL, index="year", columns="new_province", values="num",aggfunc=[np.sum])
...
cursor.execute('delete from industry_develop_index where index_num ="1_7"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='1_7'
'''
cursor.execute(sql)
