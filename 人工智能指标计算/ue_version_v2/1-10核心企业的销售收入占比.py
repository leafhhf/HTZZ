...
for f in range(0,len(field_list)):
   ...
   # 第二步  再读取当地全部企业销售收入
	...
	report_year,
	province,
	sum( total_sales ) AS total_num 
    FROM
	dm_company_annual_report_caibao 
    WHERE
	NOT isnull( province ) 
	AND province != "" 
	AND total_sales != "0" 
...
cursor.execute('delete from industry_develop_index where index_num ="1_10"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='1_10'
'''
cursor.execute(sql)
