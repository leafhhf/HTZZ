...
for field in field_list:
    sql_cmd ='''select province,year,sum(funding) as num  from (
    select * from(
    select name,province,found_year from  kjb_dc_company where label like "%{}%" and not isnull(found_year)  and not isnull(province) ) t   join ai_hushen_caibao p on t.name = p.companyname
    where not isnull(funding) and funding != "0" )  y  where year >= 2010 group by  province,year
    '''.format(field)
