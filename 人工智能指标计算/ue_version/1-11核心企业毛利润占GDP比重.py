...
for field in field_list:
    ...
    if len(df_SQL1) > 0:
        print("√" * 50)
        print("读取{}核心企业毛利润数据成功".format(field))
        print("√" * 50)
    else:
        print("X" * 50)
        print("读取{}核心企业毛利润数据失败".format(field))
        print("X" * 50)

   # 第二步  再读取GDP
    sql_cmd2 = '''select SUBSTR(`year`,1,4) as year,province,GDP from ai_gdp_data where SUBSTR(`year`,1,4) >2015 '''
    ...
    if len(df_SQL1) > 0:
        print("√" * 50)
        print("读取{}核心企业经济规模数据成功".format(field))
        print("√" * 50)
    else:
        print("X" * 50)
        print("读取{}核心企业经济规模数据失败".format(field))
        print("X" * 50)
