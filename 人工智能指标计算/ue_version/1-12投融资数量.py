...
for field in field_list:
    sql_cmd = '''select province,year,round(sum(cleaned_money),2) as money from (SELECT
        a.*,
        b.province,
        b.found_year 
    FROM
    (SELECT cid,company_name,
            substr( report_date, 1, 4 ) AS YEAR,
            cleaned_money 
        FROM
            `ue_company_finance` 
        WHERE
            cid IN ( SELECT cid FROM kjb_dc_company WHERE label LIKE "%{}%" ) 
            AND NOT isnull( cleaned_money ) 
            AND cleaned_money != "" ) a
	LEFT JOIN kjb_dc_company b ON a.cid = b.cid) c group by province,year  order by YEAR desc,money desc
        '''.format(field)
