...
for f in range(0,len(field_list)):
    sql_cmd = '''
    select report_year as funding_year,province,found_year,round(sum(num)) as money from ( SELECT a.report_year,cleaned_money * {} as num ,b.province,b.found_year FROM(
	SELECT *
	FROM kjb_de_company_finance  
	WHERE
        label LIKE "%{}%" 
        AND NOT isnull( cleaned_money ) 
        AND cleaned_money != ""
        and province != ""
        and not isnull (province) 
        
		AND NOT isnull( cleaned_money ) 
		AND cleaned_money != "" and report_year >2014) a
	LEFT JOIN kjb_de_company b ON a.cid = b.cid) t where not isnull (province) group by report_year,province,found_year
    ...
    if len(df_SQL) > 0:
        ...
        for year in list_year:
            ...
            df_SQL1["calculate_year"] = year
            df_SQL_all = pd.concat([df_SQL1, df_SQL_all], axis=0, ignore_index=True)

        df_group = df_SQL_all.groupby(by=["calculate_year", "new_province","funding_year"]).agg({"money":sum}).reset_index()
        #将计算年份以及对应年份企业的投资金额
        df_group = df_group.loc[df_group["calculate_year"]==df_group["funding_year"]]

        df_pivot = pd.pivot_table(df_group, index="calculate_year", columns="new_province", values="money", aggfunc=np.sum)
        ...
        for i in range(0, df.shape[1]):
            if int(a[i]) >= 2016 and int(a[i]) < now_year:
                ...
                df2["index_name"] = "初创企业的融资金额总量增长率"
                df2["index_num"] = "2_11"
                df2["index_id"] = "37"
                df2["type"] = "1"
                df2["field"] = field_list[f]
                df2["value"] = 0.0
...
cursor.execute('delete from industry_develop_index where index_num ="2_11"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='2_11'
'''
cursor.execute(sql)
