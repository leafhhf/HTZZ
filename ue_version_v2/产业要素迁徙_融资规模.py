...
sql_cmd ='''select *  from kjb_de_company_finance where label  '''
...
similarity_list = ['similarity_2','similarity_9','similarity_10','similarity_7','similarity_4','similarity_3',
                   'similarity_6','similarity_11','similarity_5','similarity_sum']
for f in range(0,len(field_list)):
    sql_cmd = '''
        select report_year as funding_year,province,round(sum(num)) as money from ( SELECT a.report_year,cleaned_money * {} as num ,b.province,b.found_year FROM(
    	SELECT *
    	...
    	LEFT JOIN kjb_de_company b ON a.cid = b.cid) t group by report_year,province,found_year
    ...
    if len(df_SQL) > 0:
        ...
        df_SQL = df_SQL.loc[df_SQL["new_province"] != ""]
        df_SQL["funding_year"] = df_SQL["funding_year"].astype("int")

        df_pivot = pd.pivot_table(df_SQL, index="funding_year", columns="new_province", values="money")
...
cursor.execute("delete from industry_develop_index where index_num ='5_0'")
...
sql1 = '''
    ...
    select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='5_0'
    '''
cursor.execute(sql1)
...
select year,b.name,b.city,b.longitude,b.latitude,value,sequence,field ,original_self_value,original_max_value,index_id,index_name,index_num from industry_develop_index a  left join capital_migrate_table_sup b on a.name = b.name where index_num ="5_0"
