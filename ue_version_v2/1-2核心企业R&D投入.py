...
for f in range(0,len(field_list)):
	...
	SELECT YEAR
		...
		funding * {} AS funding 
	FROM
		( SELECT * FROM kjb_de_company WHERE label LIKE "%{}%" AND NOT isnull( found_year ) AND NOT isnull( province ) ) t
		JOIN ai_hushen_caibao p ON t.NAME = p.companyname 
	WHERE
		NOT isnull( funding ) 
		AND funding != "0" 
	) y 
    ...
    if len(df_SQL) > 0:
        ...
        df_pivot = pd.pivot_table(df_SQL, index="YEAR", columns="new_province",values="num")
        df = df_pivot.T
        df = df.fillna(0)
        a =df.columns
...
cursor.execute('delete from industry_develop_index where index_num ="1_2"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='1_2'
'''
cursor.execute(sql)
