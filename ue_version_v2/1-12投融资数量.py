...
FROM
	...
	FROM
		...
		SELECT
			...
			cleaned_money 
		FROM
			kjb_de_company_finance 
		WHERE
			cid IN ( SELECT cid FROM kjb_de_company WHERE label LIKE "%{}%" ) 
...
ORDER BY
    ...
    if len(df_SQL) > 0:
        ...
        df_pivot = pd.pivot_table(df_SQL, index="YEAR", columns="new_province", values="money", aggfunc=np.sum)
...
cursor.execute('delete from industry_develop_index where index_num ="1_12"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='1_12'
'''
cursor.execute(sql)
