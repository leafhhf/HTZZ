...
for field in field_list:
	...
	kjb_de_company
    WHERE
	 label  LIKE  '%{}%'  and not isnull(province) and province != ''  and not isnull(found_year) and found_year != '' 
	 '''.format(field)
...
cursor.execute('delete from industry_develop_index where index_num ="2_1"')
...
select year,name,value,original_self_value,original_max_value,index_id,index_num,index_name,sequence,type,field from industry_develop_index_ue where index_num ='2_1'
'''
cursor.execute(sql)
