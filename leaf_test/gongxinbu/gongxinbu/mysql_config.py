...
def insert_db(self, db_table, item, spider):
    ...
    params = [item['title'], item['publishTime'],item['url'], item['filenumbername'],item['createdate'],item['themename'],item['puborg'],item['pubjigou'],item['columnname'],item['content']]
  #  print('params',params)
   # cursor = self.conn.cursor()
    try:
        self.cur.execute(
            'insert into gongxinbu_policy(title,publishTime,url,pcode,ptime,childtype ,puborg ,pubjigou,colname,content) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',params)
        self.conn.commit()
    except Exception as e:
