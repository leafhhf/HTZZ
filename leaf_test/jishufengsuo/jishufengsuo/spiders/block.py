# -*- coding: utf-8 -*-
import scrapy
from jishufengsuo.items import JishufengsuoItem
import datetime
import  time
...
class BlockSpider(scrapy.Spider):
    name = 'block'
    # allowed_domains = ['https://so.chinaz.com/']
    start_urls = ['https://so.chinaz.com/search.aspx?keyword=%E7%BE%8E%E5%9B%BD%E6%8A%80%E6%9C%AF%E5%B0%81%E9%94%81']
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//ul[@id='ulcontent']/li")
        print('trlist',trlist)
        for tr in trlist:
            item = JishufengsuoItem()
            item['title'] = tr.xpath(".//div[@class='con']/p/@title").extract_first()
            item['url'] = tr.xpath(".//div[@class='single-imgbd']/a/@href").extract_first()
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath("//div[@id='article-content']/p | //ul[@id='newstext']/p ")
        print(content)
        for cn in content:
