...
#,item['content'],item['content_img']
  ...
  #  print('content_img111111111', item['content_img'])
    ...
    params = [item['title'], item['report_time'],item['url'],item['content'],"技术封锁",'日本']
    #print('params',params)
    cursor = self.conn.cursor()
    try:
        com = self.cur.execute(
            'insert into block_lawsuit_incident(news_title, reporttime,news_url, content,type,country) values (%s,%s,%s,%s,%s,%s)',
            params)
        self.conn.commit()
    except Exception as e:
