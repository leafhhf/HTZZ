import scrapy
from yitutech.items import YitutechItem
...
class YituSpider(scrapy.Spider):
    # 一定要一个全局唯一的爬虫名称，命令行启动的时候需要指定该名称
    name = 'yitu'
    allowed_domains = ['yitutech.com']
    start_urls = ['https://www.yitutech.com/cn/news?page=1', ]
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
        'Accept-Encoding': 'gzip, deflate, br',
        'Accept-Language': 'zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2',
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:74.0) Gecko/20100101 Firefox/74.0',
        'Host': 'www.yitutech.com',
        ...
        'Referer': 'https://www.yitutech.com',
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//div[@class ='view-content']/ul/li")
        print('trlist',trlist)
        for tr in trlist:
            item = YitutechItem()
            item['title'] = tr.xpath(".//h3/text()").extract_first()
            item['url'] = tr.xpath("./a/@href").extract_first()
           # item['imgurl'] = response.urljoin(item['imgurl'])
            item['url'] = response.urljoin(item['url'])
            item['report_time'] = tr.xpath("./a/div[@class='content']/div[@class='news-description clearfix']/span/text()").extract_first().strip()
            print('打印',item)
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath("//div[@class='middle']/div[@id='nr']/p")
        print(content)
        for cn in content:
