...
#,item['content'],item['content_img']
  ...
  #  print('content_img111111111', item['content_img'])
    ...
    params = [item['title'],item['url'], item['report_time'],item['content'],"智能家居平台",'北京小米移动软件有限公司',item['pic1_url'],item['pic2_url'],item['pic3_url']]
    #print('params',params)
    cursor = self.conn.cursor()
    try:
        com = self.cur.execute(
            'insert into ai_chuangxin_platform_news_xiaomi(title, platform_news_url, report_time, content, platform_name,company_name,pic1_url,pic2_url,pic3_url) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)',
            params)
        self.conn.commit()
    except Exception as e:
