# -*- coding: utf-8 -*-
import scrapy
from sensetime.items import SensetimeItem
import datetime
import  time
...
class StSpider(scrapy.Spider):
    name = 'st'
    # allowed_domains = ['sensetime.com']
    start_urls = ['https://www.sensetime.com/cn/news-press-release']
    headers = {
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//div[@class='list cf']/ul[@class='cf ppic_ul']/li ")
        print('trlist',trlist)
        for tr in trlist:
            item = SensetimeItem()
            item['title'] = tr.xpath(".//div[@class='sub-tit ellipsis-2 case-detail-p1']/text()").extract_first()
            item['url'] = tr.xpath(".//div[@class='img']/a/@href").extract_first()
           # item['imgurl'] = response.urljoin(item['imgurl'])
            item['url'] = response.urljoin(item['url'])
            item['report_time'] = tr.xpath(".//div[@class='time arial fnt-12']/text()").extract_first().strip()
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath("//div[@class='ns-con-wrap cf']/p")
        print(content)
        for cn in content:
