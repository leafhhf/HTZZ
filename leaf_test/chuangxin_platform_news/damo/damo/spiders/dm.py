# -*- coding: utf-8 -*-
import scrapy
from damo.items import DamoItem
import datetime
import  time
...
class DmSpider(scrapy.Spider):
    name = 'dm'
    # allowed_domains = ['damo.alibaba.com']
    # start_urls = ['http://damo.alibaba.com/']
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//div[@id='swiper-container-2']/div[@class='swiper-wrapper']/*")
        print('trlist',trlist)
        for tr in trlist:
            item = DamoItem()
            item['title'] = tr.xpath(".//div[@class='news-item']/div[@class='ch-title font-serif']/a/text()").extract_first().strip()
            item['url'] = tr.xpath(".//div[@class='view-link']/a/@href").extract_first()
           ...
           #      item['url'] = url1
           #  else:
           #      item['url'] = response.urljoin(url1)

            item['report_time'] = tr.xpath(".//div[@class='date']/text()").extract_first().strip()
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath("//div[@class='markdown-body']/p")
        ...
        if content:
            ...
            content = response.xpath("//div[@class='article-inner markdown-body']/p")
            for cn in content:
                # item["content"] = cn.xpath("./p").extract_first()
                img = cn.xpath('.//img/@src')
                text = cn.xpath('.//text()').extract()
