# -*- coding: utf-8 -*-
import scrapy
from jd_neuhub.items import JdNeuhubItem
import datetime
import  time

class JdSpider(scrapy.Spider):
    name = 'jd'
    # allowed_domains = ['neuhub.jd.com']
    start_urls = ['https://neuhub.jd.com/advanced?pageNum=1']
    headers = {
    ...
    #     for i in range(1, 5):
    #         url = 'https://neuhub.jd.com/advanced?pageNum={}'.format(i)
    #         print(url)
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//div[@class='advanced']/ul/li")
        print('trlist',trlist)
        for tr in trlist:
            item = JdNeuhubItem()
            item['title'] = tr.xpath(".//div[@class='li-title']/a/text()").extract_first()
            item['url'] = tr.xpath(".//div[@class='li-title']/a/@href").extract_first()
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath("//div[@class='intro'] | //div[@class='content fixck']/p")
        print(content)
        for cn in content:
