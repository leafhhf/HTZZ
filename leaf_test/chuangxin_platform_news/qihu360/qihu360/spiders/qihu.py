...
import scrapy
from qihu360.items import Qihu360Item
import datetime
import  time
...
class QihuSpider(scrapy.Spider):
    ...
    start_urls = ['https://b.360.cn/about/news/']
    headers = {
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//div[@class='tab-container']/div/ul[@class='list']/li")
        print('trlist',trlist)
        for tr in trlist:
            item = Qihu360Item()
            item['title'] = tr.xpath(".//img/@alt").extract_first()
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath(".//article[@class='news-content fullwidth']/section/section/p |//article[@class='news-content fullwidth']/p | //article[@class='news-content fullwidth']/section//section/section/section/section/p")
        print(content)
        for cn in content:
