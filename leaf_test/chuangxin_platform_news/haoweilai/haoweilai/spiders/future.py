...
import scrapy
from haoweilai.items import HaoweilaiItem
import datetime
import  time

class FutureSpider(scrapy.Spider):
    name = 'future'
    # allowed_domains = ['100tal.com']
    start_urls = ['http://www.100tal.com/news1_1.html']
    headers = {
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//div[@class='fix-width']/ul/li")
        print('trlist',trlist)
        for tr in trlist:
            item = HaoweilaiItem()
            item['title'] = tr.xpath("./p/a/text()").extract_first()
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath("//div[@class='newscon']/p")
        print(content)
        for cn in content:
