# -*- coding: utf-8 -*-
import scrapy
from kuangshi.items import KuangshiItem
import datetime
import  time

class KsSpider(scrapy.Spider):
    name = 'ks'
    allowed_domains = ['megvii.com']
    start_urls = ['https://www.megvii.com/news/?cate=1&page=1']
    ...
    def parse(self, response):
        ...
        trlist = response.xpath("//div[@class='news-container page-content']/div/a[@class = 'news-item']")
        print('trlist',trlist)
        for tr in trlist:
            item = KuangshiItem()
            item['title'] = tr.xpath(".//div[@class='title']/text()").extract_first()
            item['url'] = tr.xpath(".//@href").extract_first()
    ...
    def parse_detail(self, response):  # 处理详情页
        ...
        content = response.xpath(".//div[@class='text_box']/p")
        print(content)
        for cn in content:
