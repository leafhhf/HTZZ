...
#,item['content'],item['content_img']
  ...
  #  print('content_img111111111', item['content_img'])
    ...
    params = [item['title'],item['url'], item['report_time'],item['content'],"美国财政部treasury——pr",'美国']
    #print('params',params)
    cursor = self.conn.cursor()
    try:
        com = self.cur.execute(
            'insert into block_lawsuit_incident_treasury_pr(news_title, news_url, reporttime, content, source,country) values (%s,%s,%s,%s,%s,%s)',
            params)
        self.conn.commit()
    except Exception as e:
