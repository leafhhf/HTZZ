                ...
                for i in range(len(p)):
                    for j in range(min(len(p[i]),3)):
                        ...
                        if p[i][j] is None or p[i][j] == '':
                            continue
                        #print('p[i][j]',p[i][j])
                        if table_header_yftr[1] in p[i][j] or table_header_yftr[1] in p[i][j]:
...
def match_yftr2(all_pages):
    ...
    #
    print('list_page_yftr',list_page_yftr)
    # yftr = {}
    #
    # list_zcqk = []
    #
...
# 更换需要解析的pdf的相对路径即可
pages = parse_pages('PDF/ST升达(002259.SZ)_2018-04-27.pdf')
#
...
def write_to_dict_6_2(result_dir, list, time):
    ...
    if list[0][0] == '':
        ...
        #     dict_temp1 = {}
        #     dict_temp2 = {}
        #     dict_temp1['date'] = str(int(time))
        #     dict_temp2['date'] = str(int(time) - 1)
        #     for i in range(len(list)):
        #         if i == 0:
        #             dict_temp1['date'] = split_kongge(list[i][1])
        #             dict_temp2['date'] = split_kongge(list[i][2])
        ...
        #     dict_temp1['date'] = str(int(time))
        #     dict_temp2['date'] = str(int(time) - 1)
        #     dict_temp3['date'] = str(int(time) - 2)
        #     for i in range(len(list)):
        #         dict_temp1[split_kongge(list[i][0])] = split_kongge(list[i][1])
        #         dict_temp2[split_kongge(list[i][0])] = split_kongge(list[i][2])
        #         dict_temp3[split_kongge(list[i][0])] = split_kongge(list[i][3])
